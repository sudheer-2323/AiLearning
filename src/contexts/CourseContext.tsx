...
export function CourseProvider({ children, onNavigate, isAuthenticated }: CourseProviderProps) {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const [coursesRes, progressRes] = await Promise.all([
        axios.get<Course[]>('http://127.0.0.1:5000/api/courses', { withCredentials: true }),
        axios.get('http://127.0.0.1:5000/api/progress', { withCredentials: true }),
      ]);
  ...
  const markLectureComplete = async (courseId: string, lectureId: string) => {
    try {
      ...
      const response = await axios.post(
        'http://127.0.0.1:5000/api/progress/lecture',
        { courseId, lectureId },
  ...
  const completeQuiz = async (courseId: string, quizId: string, score: number) => {
    try {
      ...
      await axios.post(
        'http://127.0.0.1:5000/api/progress/quiz',
